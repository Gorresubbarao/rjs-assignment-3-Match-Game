...
class GameOver extends Component {
  ...
  setTimer = () => {
    this.timeInterval = setInterval(this.decreaseTime, 1000)
  }
  ...
  onClickImgeItem = imageUrl => {
    ...
    if (imageUrl !== matchedImage) {
      ...
      this.getRandomUdatedUrl()
    }
  }

  getRandomUdatedUrl = () => {
    const randamIndex = Math.ceil(Math.random() * (imagesList.length - 1))
    const randamObj = imagesList[randamIndex]
    const {imageUrl} = randamObj
    const {matchScore} = this.state
  ...
  render() {
    const {matchedImage, matchScore, timer, isGameStart} = this.state

    return (
      <div className="app-container">
        <div className="response-container">
          <Header matchScore={matchScore} timer={timer} />
          {isGameStart ? (
            <>
              <div className="display-img-container">
                <img src={matchedImage} alt="match" className="display-img" />
              </div>
              {this.tabsListView()}
              {this.imageListView()}
            </>
